...
struct FinnhubStockQuoteProvider: StockQuoteProviding {
    let apiKey: String
    ...
    func fetchQuote(symbol: String) async throws -> StockQuote {
        ...
        guard !apiKey.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else { throw StockQuoteError.missingAPIKey }

        async let quotePayload = fetchJSON(path: "quote", symbol: normalized)
        async let profilePayload = fetchJSON(path: "stock/profile2", symbol: normalized)
        let quote = try await quotePayload
        let profile = try? await profilePayload

        guard let current = quote["c"] as? Double, current > 0 else {
            throw StockQuoteError.invalidPayload
        }
        let previousClose = quote["pc"] as? Double
        let timestamp = (quote["t"] as? Double).flatMap { $0 > 0 ? Date(timeIntervalSince1970: $0) : nil }
        ...
        return StockQuote(
            ...
            regularTimestamp: timestamp,
            extendedPrice: nil,
            extendedTimestamp: nil,
            extendedChange: nil,
            extendedChangePercent: nil,
            marketState: USMarketHours.marketState(now: now),
    ...
    func validateSymbol(_ symbol: String) async -> Bool {
        guard StockPosition.isValidSymbol(symbol) else { return false }
        guard !apiKey.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else { return true }
        do {
    ...
    func searchSymbols(query: String) async throws -> [StockSearchResult] {
        ...
        guard !apiKey.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else { throw StockQuoteError.missingAPIKey }
        var comp = URLComponents(string: "https://finnhub.io/api/v1/search")!
        comp.queryItems = [
            URLQueryItem(name: "q", value: trimmed),
            URLQueryItem(name: "token", value: apiKey)
        ]
        guard let url = comp.url else { throw StockQuoteError.invalidPayload }
        do {
            let (data, response) = try await Self.session.data(from: url)
            guard let http = response as? HTTPURLResponse else { throw StockQuoteError.invalidResponse }
            if http.statusCode == 429 { throw StockQuoteError.rateLimited }
            guard (200..<300).contains(http.statusCode) else { throw StockQuoteError.invalidResponse }
            ...
            return results.compactMap { item in
                ...
                let description = item["description"] as? String ?? symbol
                let displaySymbol = item["displaySymbol"] as? String ?? symbol
                return StockSearchResult(
                    symbol: StockPosition.normalizeSymbol(symbol),
                    description: description,
                    displaySymbol: displaySymbol,
                    type: type
                )
            }
            .filter { $0.type.uppercased().contains("COMMON") || $0.type.uppercased().contains("EQUITY") || $0.type.isEmpty }
            .prefix(8)
            .map { $0 }
    ...
    private func fetchJSON(path: String, symbol: String) async throws -> [String: Any] {
        var comp = URLComponents(string: "https://finnhub.io/api/v1/\(path)")!
        comp.queryItems = [
...
struct AlphaVantageStockQuoteProvider: StockQuoteProviding {
    let apiKey: String
    ...
    func fetchQuote(symbol: String) async throws -> StockQuote {
        ...
        let obj = try await fetchJSON(queryItems: [
            URLQueryItem(name: "function", value: "GLOBAL_QUOTE"),
            URLQueryItem(name: "symbol", value: normalized),
            URLQueryItem(name: "apikey", value: apiKey)
        ])
        try checkAPIMessage(obj)
        guard let quote = obj["Global Quote"] as? [String: Any],
              let price = parseDouble(quote["05. price"]),
              price > 0 else {
            throw StockQuoteError.invalidPayload
        }

        let previousClose = parseDouble(quote["08. previous close"])
        let latestTradingDay = quote["07. latest trading day"] as? String
        let timestamp = latestTradingDay.flatMap { Self.tradingDayFormatter.date(from: $0) }
        let now = Date()
    ...
    func searchSymbols(query: String) async throws -> [StockSearchResult] {
        ...
        try checkAPIMessage(obj)
        guard let matches = obj["bestMatches"] as? [[String: Any]] else {
            throw StockQuoteError.invalidPayload
        }

        return matches.compactMap { item in
            guard let symbol = item["1. symbol"] as? String, StockPosition.isValidSymbol(symbol) else { return nil }
            let type = item["3. type"] as? String ?? ""
            let region = item["4. region"] as? String ?? ""
            let name = item["2. name"] as? String ?? symbol
            return StockSearchResult(
                symbol: StockPosition.normalizeSymbol(symbol),
                description: name,
                displaySymbol: symbol,
                type: [type, region].filter { !$0.isEmpty }.joined(separator: " · ")
            )
        }
        .filter { result in
            let upperType = result.type.uppercased()
            return upperType.contains("EQUITY") || upperType.contains("UNITED STATES") || upperType.isEmpty
        }
        .prefix(8)
        .map { $0 }
    }

    private func fetchJSON(queryItems: [URLQueryItem]) async throws -> [String: Any] {
        var comp = URLComponents(string: "https://www.alphavantage.co/query")!
    ...
    private func checkAPIMessage(_ obj: [String: Any]) throws {
        if obj["Note"] != nil || obj["Information"] != nil {
    ...
    private static let tradingDayFormatter: DateFormatter = {
...
struct MockStockQuoteProvider: StockQuoteProviding {
    let providerName = "Mock"

    func fetchQuote(symbol: String) async throws -> StockQuote {
        let normalized = StockPosition.normalizeSymbol(symbol)
        guard StockPosition.isValidSymbol(normalized) else { throw StockQuoteError.invalidSymbol }
        let base = mockBasePrice(for: normalized)
        let previous = base * 0.982
        let now = Date()
        return StockQuote(
            symbol: normalized,
            name: mockName(for: normalized),
            regularPrice: base,
            previousClose: previous,
            regularTimestamp: now,
            extendedPrice: nil,
            extendedTimestamp: nil,
            extendedChange: nil,
            extendedChangePercent: nil,
    ...
    func searchSymbols(query: String) async throws -> [StockSearchResult] {
        let normalized = StockPosition.normalizeSymbol(query)
        guard !normalized.isEmpty else { return [] }
        let candidates = ["NVDA", "MU", "AAPL", "TSLA", "MSFT", "GOOGL", "AMD", "AVGO"]
        return candidates
            .filter { $0.contains(normalized) || mockName(for: $0).uppercased().contains(normalized) }
            .map {
                StockSearchResult(
                    symbol: $0,
                    description: mockName(for: $0),
                    displaySymbol: $0,
                    type: "Common Stock"
                )
            }
    ...
    private func mockBasePrice(for symbol: String) -> Double {
        switch symbol {
        case "NVDA": return 128.40
        case "MU": return 93.20
        case "AAPL": return 196.10
        case "TSLA": return 182.75
        default:
            let seed = Double(abs(symbol.hashValue % 9000)) / 100
            return max(10, seed)
        }
    }

    private func mockName(for symbol: String) -> String {
        switch symbol {
